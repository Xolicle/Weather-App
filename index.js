...
function displayForecast(response) {
  ...
  forecast.forEach((forecastDay, index) => {
    ...
    let maxTemp = day + "Max";
    let minTemp = day + "Min";
    document.getElementById(day).innerHTML = weekDay(forecastDay.dt);
    document.getElementById(img).src = `https://openweathermap.org/img/wn/${forecastDay.weather[0].icon}.png`; 
    document.getElementById(maxTemp).innerHTML = `${Math.round(forecastDay.temp.max)}° `;
    document.getElementById(minTemp).innerHTML = ` ${Math.round(forecastDay.temp.min)}°`;
    
  });

}
function getForecast(coords) {
  let forecastUrl = `https://api.openweathermap.org/data/2.5/onecall?lat=${coords.lat}&lon=${coords.lon}&units=metric&appid=${apiKey}`;
